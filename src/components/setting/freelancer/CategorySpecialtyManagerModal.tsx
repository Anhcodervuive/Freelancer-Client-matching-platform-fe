...
const CATEGORY_FETCH_LIMIT = 10
const SPECIALTY_FETCH_LIMIT = 50
...
export default function CategorySpecialtyManagerModal({
	...
	const categories = useMemo(
		() => (categoriesQuery.data?.data ?? []).slice().sort((a: Category, b: Category) => a.name.localeCompare(b.name)),
	...
	const specialtiesQuery = useQuery({
		...
		queryFn: () => getSpecialties({ page: 1, limit: SPECIALTY_FETCH_LIMIT, search: '' })
	})
	...
	const ensureCategorySelected = (categoryId: string) => {
		...
		setSelectedCategories(prev => {
			...
			setExpandedCategories(current => (current.includes(categoryId) ? current : [...current, categoryId]))
										...
										<div key={category.id} className='rounded-xl border border-base-200'>
											<button
												...
												<div>
													...
													<div className='text-xs text-base-content/60'>{items.length} chuyên môn</div>
												</div>
														...
														<div className='flex flex-wrap gap-2'>
															{items.map(item => {
																...
																const disableSpecialty =
																	!isChecked && selectedSpecialties.length >= FREELANCER_LIMITS.maxSpecialties
																return (
																	<label
																		key={item.id}
																		className={`flex items-center gap-2 rounded-full border px-3 py-1 text-sm transition ${
																			isChecked ? 'border-primary bg-primary/10 text-primary' : 'border-base-200'
																		} ${saving ? 'opacity-50' : ''}`}>
																		<input
																			type='checkbox'
																			className='checkbox checkbox-sm'
