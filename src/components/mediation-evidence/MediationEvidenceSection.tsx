...
import { listMediationEvidenceSubmissions, createMediationEvidenceSubmission } from '~/apis/mediation-evidence.api'
...
export default function MediationEvidenceSection({
	...
	const submitEvidenceMutation = useMutation({
		mutationFn: (data: CreateMediationEvidenceSubmissionInput) => createMediationEvidenceSubmission(disputeId, data),
		onSuccess: () => {
				...
				<nav className='-mb-px flex space-x-8'>
					...
					<button
						...
						<div className='flex items-center space-x-2'>
							...
							{proposals.length > 0 && (
								<span className='bg-gray-100 text-gray-600 py-0.5 px-2 rounded-full text-xs'>{proposals.length}</span>
							)}
						</div>
					</button>
				</nav>
							...
							{hasSubmittedEvidence ? (
								<div className='bg-green-50 border border-green-200 rounded-lg p-4'>
									<div className='flex items-center space-x-3'>
										...
										<div>
											<p className='text-sm font-medium text-green-800'>Bạn đã nộp bằng chứng thành công</p>
											<p className='text-xs text-green-600 mt-1'>Admin sẽ xem xét và đưa ra đề xuất hòa giải</p>
										</div>
									</div>
								</div>
							) : (
								<div className='bg-yellow-50 border border-yellow-200 rounded-lg p-4'>
									<div className='flex items-center space-x-3'>
										...
										<div>
											<p className='text-sm font-medium text-yellow-800'>Chưa nộp bằng chứng</p>
											<p className='text-xs text-yellow-600 mt-1'>Hãy nộp bằng chứng để hỗ trợ quá trình hòa giải</p>
										</div>
									</div>
								</div>
							)}
...
function EvidenceSubmissionCard({ submission, userRole, currentUserId }: EvidenceSubmissionCardProps) {
	...
	const getSourceTypeText = (sourceType: string) => {
		switch (sourceType) {
			...
			case 'DOCUMENT_UPLOAD':
				return 'Tài liệu upload'
			case 'SCREENSHOT':
				return 'Ảnh chụp màn hình'
			case 'MILESTONE_ATTACHMENT':
				return 'Đính kèm milestone'
			case 'CHAT_ATTACHMENT':
				return 'Đính kèm chat'
			case 'CONTRACT_DOCUMENT':
				return 'Tài liệu hợp đồng'
			default:
				return sourceType
		}
	}
					...
					<div className='flex-1'>
						<div className='flex items-center space-x-3 mb-2'>
							<div className='flex items-center space-x-2'>
								...
								{isOwnSubmission && <span className='text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded'>Của bạn</span>}
							</div>
							<span
						...
						{submission.title && <h5 className='font-medium text-gray-900 mb-1'>{submission.title}</h5>}

						<div className='flex items-center justify-between'>
							<div className='text-xs text-gray-500'>
								Nộp lúc: {new Date(submission.createdAt).toLocaleString('vi-VN')}
								{submission.submittedAt && (
									<span className='ml-2'>• Gửi lúc: {new Date(submission.submittedAt).toLocaleString('vi-VN')}</span>
								)}
							</div>
							<button
										...
										<div className='flex items-start space-x-3'>
											...
											<div className='flex-1 min-w-0'>
												<div className='flex items-center justify-between mb-1'>
													...
													<span className='text-xs text-gray-500 ml-2'>{getSourceTypeText(item.sourceType)}</span>
												</div>
					...
					{submission.reviewedAt && (
						<div className='border-t border-gray-200 pt-3'>
							...
							<div className='text-xs text-gray-600'>
								...
								{submission.reviewNotes && <p className='mt-1'>Ghi chú: {submission.reviewNotes}</p>}
							</div>
						</div>
					)}
				</div>
