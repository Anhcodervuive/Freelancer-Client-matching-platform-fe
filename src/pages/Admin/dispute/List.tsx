...
const humanizeNegotiationStatus = (status?: DisputeNegotiationStatus | null) =>
        status ? status.split('_').map(part => part.charAt(0) + part.slice(1).toLowerCase()).join(' ') : '—'
                                        ...
                                        {!isLoading && !isError && disputes.length > 0 ? (
                                                <div className='space-y-4'>
                                                        {disputes.map(item => {
                                                                ...
                                                                const milestoneRecord = item.milestone
                                                                        ? (item.milestone as Record<string, unknown>)
                                                                        : null
                                                                ...
                                                                const proposedRelease =
                                                                        amounts?.proposedRelease ??
                                                                        baseDispute?.latestProposal?.releaseAmount ??
                                                                        baseDispute?.proposedRelease
                                                                const proposedRefund =
                                                                        amounts?.proposedRefund ??
                                                                        baseDispute?.latestProposal?.refundAmount ??
                                                                        baseDispute?.proposedRefund
                                                                ...
                                                                const negotiationCount =
                                                                        ...
                                                                        (baseDispute?.negotiations ? baseDispute.negotiations.length : null)
                                                                ...
                                                                const lastProposalCreatedAt =
                                                                        metrics?.lastProposalCreatedAt ?? latestProposal?.createdAt ?? null
                                                                const lastProposalRespondedAt =
                                                                        metrics?.lastProposalRespondedAt ?? latestProposal?.respondedAt ?? null
                                                                const lastAdminJoinedAt = metrics?.lastAdminJoinedAt ?? null
                                                                const fallbackNeedsAdmin =
                                                                        typeof item.needsAdmin === 'boolean'
                                                                                ? item.needsAdmin
                                                                                : pickBooleanFlag(item, [
                                                                                          'needs_admin',
                                                                                          'requiresAdmin',
                                                                                          'awaitingAdmin'
                                                                                  ]) ?? false
                                                                ...
                                                                const isResponseOverdue = Boolean(
                                                                        metrics?.isResponseOverdue ??
                                                                                (responseDeadline
                                                                                        ? new Date(responseDeadline).getTime() < Date.now()
                                                                                        : false)
                                                                )
                                                                        ...
                                                                        <article key={item.id} className='space-y-4 rounded-2xl border border-base-200 p-5 shadow-sm'>
                                                                                                ...
                                                                                                <div className='flex flex-wrap items-center gap-2'>
                                                                                                        <span
                                                                                                                className={[
                                                                                                                        'badge',
                                                                                                                        status
                                                                                                                                ? statusClassMap[status] ?? 'badge-outline'
                                                                                                                                : 'badge-outline'
                                                                                                                ].join(' ')}
                                                                                                        >
                                                                                ...
                                                                                <div className='grid gap-4 md:grid-cols-2 xl:grid-cols-4'>
                                                                                        <div className='space-y-2 text-sm'>
                                                                                                <h4 className='text-xs font-semibold uppercase tracking-wide text-base-content/60'>Hợp đồng &amp; mốc</h4>
                                                                                                <div className='space-y-1'>
                                                                                                        <div className='font-medium text-base-content'>
                                                                                                                {contractTitle || (item.contract?.id ? `Hợp đồng #${item.contract.id}` : 'Không rõ hợp đồng')}
                                                                                                        </div>
                                                                                        ...
                                                                                        <div className='space-y-3 text-sm'>
                                                                                                <h4 className='text-xs font-semibold uppercase tracking-wide text-base-content/60'>Các bên liên quan</h4>
                                                                                                <div className='flex items-start gap-2'>
                                                                                                        ...
                                                                                                        <div>
                                                                                                                ...
                                                                                                                <div className='text-xs text-base-content/60'>ID: {client?.id ?? item.contract?.clientId ?? '—'}</div>
                                                                                                        </div>
                                                                                                </div>
                                                                                                <div className='flex items-start gap-2'>
                                                                                                        <User2 className='mt-1 size-4 text-base-content/60' />
                                                                                                        <div>
                                                                                                                ...
                                                                                                                <div className='text-xs text-base-content/60'>ID: {freelancer?.id ?? item.contract?.freelancerId ?? '—'}</div>
                                                                                                        </div>
                                                                                                </div>
                                                                                        </div>

                                                                                        <div className='space-y-2 text-sm'>
                                                                                                <h4 className='text-xs font-semibold uppercase tracking-wide text-base-content/60'>Tài chính</h4>
                                                                                                <div className='space-y-1'>
                                                                                        ...
                                                                                        <div className='space-y-2 text-sm'>
                                                                                                <h4 className='text-xs font-semibold uppercase tracking-wide text-base-content/60'>Hoạt động gần đây</h4>
                                                                                                <div className='space-y-1'>
                                                                                                        <div className='flex items-center justify-between gap-2'>
