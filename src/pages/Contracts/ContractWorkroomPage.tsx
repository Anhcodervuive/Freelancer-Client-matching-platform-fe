...
const buildTimeline = (contract?: Contract | null) => {
        ...
        events.push({ id: 'created', date: contract.createdAt, label: 'Hợp đồng được tạo' })
        events.push({ id: 'proposal', date: contract.proposal?.submittedAt, label: 'Freelancer gửi proposal' })
        events.push({ id: 'offer', date: contract.offer?.createdAt, label: 'Client gửi offer' })
        ...
        events.push({ id: 'updated', date: contract.updatedAt, label: 'Cập nhật gần nhất' })

        return events.filter(event => Boolean(event.date))
}

const buildAttachmentList = (contract?: Contract | null) => {
        const attachments = contract?.jobPost?.attachments
        if (!attachments || !Array.isArray(attachments)) return [] as Array<{
                id: string
                name: string
                url?: string
                size?: number
        ...
        return attachments
                .map(attachment => {
                        ...
                        if (typeof attachment === 'string') {
                                return { id: attachment, name: attachment.split('/').pop() || attachment, url: attachment }
                        }
                ...
                .filter((item): item is { id: string; name: string; url?: string; size?: number; mimeType?: string } => Boolean(item))
}
                        ...
                        <div className='grid gap-4 rounded-[28px] border border-white/70 bg-white/85 p-6 shadow-[0_25px_70px_rgba(15,23,42,0.08)] md:grid-cols-3 md:p-8'>
                                <div className='space-y-3'>
                                        ...
                                        <div className='inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-semibold'
                                                ...
                                                >
                                                <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-sm font-semibold ${statusMeta.badge} ${statusMeta.text}`}>
                                ...
                                <div className='space-y-3'>
                                        ...
                                        <ul className='space-y-2 text-sm text-slate-600'>
                                                ...
                                                {hourlyRate && <li>Đơn giá theo giờ: <span className='font-semibold text-slate-800'>{hourlyRate}</span></li>}
                                                {fixedPrice && <li>Giá trị hợp đồng: <span className='font-semibold text-slate-800'>{fixedPrice}</span></li>}
                                                {totalPaid && <li>Đã thanh toán: <span className='font-semibold text-slate-800'>{totalPaid}</span></li>}
                                                {outstanding && <li>Còn lại: <span className='font-semibold text-slate-800'>{outstanding}</span></li>}
                                        </ul>
                                </div>
                                <div className='space-y-3'>
                                        <p className='text-xs font-semibold uppercase tracking-[0.3em] text-slate-400'>Mốc thời gian</p>
                                        <ul className='space-y-2 text-sm text-slate-600'>
                                                <li>Bắt đầu: <span className='font-semibold text-slate-800'>{formatDateTime(contract?.startDate || contract?.offer?.startDate, { dateStyle: 'medium' }) ?? '—'}</span></li>
                                                <li>Kết thúc: <span className='font-semibold text-slate-800'>{formatDateTime(contract?.endDate || contract?.offer?.endDate, { dateStyle: 'medium' }) ?? '—'}</span></li>
                                                <li>Tạo hợp đồng: <span className='font-semibold text-slate-800'>{formatDateTime(contract?.createdAt, { dateStyle: 'medium', timeStyle: 'short' }) ?? '—'}</span></li>
                                        </ul>
                                </div>
                        </div>
                        ...
                        <div className='rounded-[28px] border border-white/70 bg-white/85 p-6 shadow-[0_25px_70px_rgba(15,23,42,0.08)]'>
                                <div className='flex flex-col gap-4 md:flex-row md:items-start md:justify-between'>
                                        <div className='space-y-3'>
                                                ...
                                                <p className='text-base font-semibold text-slate-900'>{contract?.jobPost?.title ?? contract?.title ?? 'Chưa cập nhật tiêu đề'}</p>
                                ...
                                {milestones.map(milestone => {
                                        ...
                                        return (
                                                ...
                                                >
                                                        ...
                                                        <div className='mt-4 text-xs text-slate-400'>Tạo ngày {formatDateTime(milestone.createdAt, { dateStyle: 'medium', timeStyle: 'short' }) ?? '—'}</div>
                                                </div>
                                        )
                                })}
                        </div>
                ...
                if (!attachments.length) {
                        return (
                                <div className='rounded-[28px] border border-dashed border-slate-200 bg-white/80 p-10 text-center text-slate-500 shadow-inner shadow-white/30'>
                                        ...
                                        <p className='mt-2 text-sm text-slate-500'>Hãy tải tài liệu liên quan tới hợp đồng để cả hai bên dễ dàng truy cập.</p>
                                </div>
                        )
                }
                ...
                <div className='grid gap-6 md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]'>
                        <div className='space-y-6 rounded-[28px] border border-white/70 bg-white/85 p-6 shadow-[0_25px_70px_rgba(15,23,42,0.08)]'>
                                ...
                                <div className='space-y-3 rounded-2xl border border-white/70 bg-white/80 p-4 text-sm text-slate-600'>
                                        ...
                                        <p>Quản lý thanh toán trực tiếp trong Workroom. Các khoản thanh toán sẽ hiển thị tại đây khi bạn phát hành milestone.</p>
                                        <p className='text-xs text-slate-400'>Lưu ý: hệ thống thanh toán đang được tích hợp, thông tin hiện tại chỉ mang tính mô phỏng.</p>
                                </div>
                        </div>
                        <div className='space-y-4 rounded-[28px] border border-dashed border-slate-200 bg-white/80 p-6 text-sm text-slate-500 shadow-inner shadow-white/30'>
                                ...
                                <p>Chưa có giao dịch nào được ghi nhận. Khi milestones được duyệt và giải ngân, bạn sẽ thấy chi tiết tại đây.</p>
                        </div>
                </div>
        )
                ...
                if (!timelineEvents.length) {
                        return (
                                <div className='rounded-[28px] border border-dashed border-slate-200 bg-white/80 p-10 text-center text-slate-500 shadow-inner shadow-white/30'>
                                        ...
                                        <p className='mt-2 text-sm text-slate-500'>Những cập nhật của hợp đồng sẽ hiển thị tại đây theo trình tự thời gian.</p>
                                </div>
                        )
                }
                        ...
                        <div className='flex flex-col gap-4 rounded-[38px] border border-white/60 bg-gradient-to-br from-primary/10 via-white to-secondary/20 p-6 shadow-[0_30px_110px_rgba(15,23,42,0.1)] md:flex-row md:items-center md:justify-between md:p-10'>
                                <div className='space-y-4'>
                                        ...
                                        <div className='inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold'>
                                                ...
                                                <span className='text-slate-500'>Bắt đầu {formatDateTime(contract?.startDate || contract?.offer?.startDate, { dateStyle: 'medium' }) ?? '—'}</span>
                                        </div>
                                </div>
